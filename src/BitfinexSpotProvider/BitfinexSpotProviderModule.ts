...
import { bitfinexSpotHttpProviderKey, bitfinexSpotProviderKey, bitfinexSpotApiUrl } from './constants';
...
@Module({
  providers: [
        ...
        httpClient.createHttpClient({
          ...
          onRequest: async config => {
            ...
            if (publicKey && [ExchangeSecurityTypeEnum.KEY, ExchangeSecurityTypeEnum.FULL].includes(<ExchangeSecurityTypeEnum>securityType)) {
              headers['bfx-apikey'] = publicKey;
            }

            if (secretKey && securityType === ExchangeSecurityTypeEnum.FULL) {
              const timestamp = (Date.now() * 1000).toString();
              headers['bfx-nonce'] = timestamp;
              headers['bfx-signature'] = crypto
                .createHmac('sha384', <string>secretKey)
                .update(
                  decodeURIComponent(
                    `/api${new URL(config.url).pathname}${timestamp}${
                      config.body
    ...
    {
      provide: bitfinexSpotProviderKey,
      useFactory: (
  ...
  exports: [bitfinexSpotProviderKey],
})
export class BitfinexSpotProviderModule {}
